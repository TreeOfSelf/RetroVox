...
//Amount of cull data we are sending
var sendAmount=0
//List of messages we are going to send 
var sendList=[];

//How many chunks to cull out
var view =5;
//How far to draw out
var viewDist=10;
var zView=4;
...
//Keeps track of faces culled
var culledList=[];

//Used to see if a layer needs to be culled
var chunkLayersDrawn=[];
//Generate chunk layer array
var chunkLayers=[];

//Generate list for layers being drawn
var chunkLayersDrawn=[];
...
chunk_create = function(x,y,z){
	//console.log("%c Created chunk: %c"+x+','+y+','+z,'color:green; font-weight:bold;','color:black')
	...
	if(chunk[chunkID]==null){
		...
		chunk[chunkID]={
			...
			//marked to cull or not 
			toCull : chunkList.slice(),
			//list of culled blocks
			culledList : culledList.slice(),
			//whether or not a layer needs to be culled
			blockLayersCulled : chunkLayersDrawn.slice(),
			//Whether the chunk needs to be redrawn
			chunkreDraw : 0,
			
			fullyCulled : 0,
			//layers containing draw data when drawing 3D
			blockLayersCube : [],
			//whether or not a layer needs to be redrawn
			blockLayersDrawn : chunkLayersDrawn.slice(),
			
		}
		
		for(var i=0;i<chunkZ;i++){
			//For each layer of a chunk
			chunk[chunkID].blockLayersCube[i]={
				//Map position XYZ
				position :[],
				//Color RGB
				color : [],
...
set_culled = function(xxx,yyy,zzz){
		//Get location of block 
	var locGet = location_block(xxx,yyy,zzz);
		//if it exists
	if(locGet!=-1){
		//Set it to be culled 
		chunk[locGet[0]].toCull[locGet[1]]=1;
		chunk[locGet[0]].fullyCulled=0;
		chunk[locGet[0]].blockLayersCulled[zzz-(chunk[locGet[0]].coords[2]*chunkZ)]=0;
		chunk[locGet[0]].chunkreDraw=1;			
	}
}
...
block_create = function(x,y,z,dontCull){
	...
	if(chunk[chunkID].blockList[blockIndex]==0){
		...
		//If this is a block that is to be culled
		if(dontCull==0){
		//set surroudning blocks to be culled
		set_culled(x+1,y,z);
		set_culled(x-1,y,z);
		set_culled(x,y+1,z);
		set_culled(x,y-1,z);
		...
		//this is for special cases where you know the block will be culled (center of a cube)
		chunk[chunkID].culledList[blockIndex]=0;	
		}
	//If there is a block here
	}	else	{
		
	//Check all the blocks around it for culling if a block exists here
	if(dontCull==1 && chunk[chunkID].culledList[blockIndex]!=0){
		
		if(blockLoc[0]==chunkXY-1){
		set_culled(x+1,y,z);
		}
		...
		if(blockLoc[1]==chunkXY-1){
		set_culled(x,y+1,z);
		}
		if(blockLoc[1]==0){
		set_culled(x,y-1,z);
		}
		if(blockLoc[2]==chunkZ-1){
		set_culled(x,y,z+1);
		}
		if(blockLoc[2]==0){
		set_culled(x,y,z-1);
...
block_delete = function(x,y,z){
	...
	chunk[chunkID].blockLayersDrawn[blockLoc[2]]=0;
	//set surroudning blocks to be culled
	for(var xZ=-1; xZ<=1;xZ++){
	for(var yZ=-1; yZ<=1;yZ++){
	for(var zZ=-1; zZ<=1;zZ++){
		set_culled(x+xZ,y+yZ,z+zZ);
	}}}
	chunk[chunkID].toCull[blockIndex]=1;
	//Set chunk the block is in to be culled
	chunk[chunkID].fullyCulled=0;
	chunk[chunkID].blockLayersCulled[blockLoc[2]]=0;
	//Set surrounding cunks to be re culled
	chunk[chunkID].blockLayersCulled[blockLoc[2]+1]=0;
	chunk[chunkID].blockLayersCulled[blockLoc[2]-1]=0;
	}
...
draw_face = function(verts,colors){
		pushPosition.push.apply(pushPosition,verts);
		pushColor.push.apply(pushColor,colors);	
}
...
//Draws chunk XY layer Z
chunk_layer_draw = function(x,y,z,q){
	var chunkID = return_chunkID(x,y,z);
		//Create fresh draw list for this layer
		posIndex=0;
		var blocksHit=0;
		//chunk[chunkID].blockLayersCube[q].position=[];
		//chunk[chunkID].blockLayersCube[q].color=[];	
		
		var startTime = new Date();
		layerDraws+=1;
		
		//Get start position of our loop check
		var loopStart = q*(chunkXY*chunkXY);
		//Loop through blocks and generate draw list
		var loopLen=(loopStart+(chunkXY*chunkXY));

		for(var k=loopStart; k<loopLen;k++){
			//If the block is not empty and it is not culled
			if( chunk[chunkID].culledList[k]!=0 && chunk[chunkID].blockList[k]!=0 ){
				//get Real Position	
				var realPos = chunkInd[k];;

				
				var pos = [(realPos[0]+(x*chunkXY))*2,(realPos[1]+(y*chunkXY))*2,(realPos[2]+(z*chunkZ))*2];
				

				var colorChange = return_color(chunk[chunkID].blockList[k]);
					blocksHit+=1;
					pushPosition=[];
					pushColor=[];
					
					//var checkNumber = [Math.floor( (chunk[chunkID].culledList[k] % 100000) / 10000 ),Math.floor( (chunk[chunkID].culledList[k] % 10000) / 1000 ),Math.floor( (chunk[chunkID].culledList[k] % 1000) / 100 )];
					var checkNumber = chunk[chunkID].culledList[k].toString();
					//bottom
					if(checkNumber[0]!=1 && checkNumber[0]!=3 ){
						...
						//Colors
						[colorChange[0],colorChange[1],colorChange[2],
						colorChange[0],colorChange[1],colorChange[2],
						colorChange[0],colorChange[1],colorChange[2],
						colorChange[0],colorChange[1],colorChange[2],
						]);
					...
					//top
					if(checkNumber[0]!=1 && checkNumber[0]!=4  ){
						
						var newColor = [colorChange[0]*1.5,colorChange[1]*1.5,colorChange[2]*1.5]
					...
					//down
					if(checkNumber[1]!=1 && checkNumber[1]!=3  ){
						var newColor = [colorChange[0]*1.4,colorChange[1]*1.4,colorChange[2]*1.4]	
						//Verts
						draw_face([
							pos[0]-1.0, pos[1]-1.0, pos[2]-1.0,
							pos[0]+1.0, pos[1]-1.0, pos[2]-1.0,
					...
					//up
					if(checkNumber[1]!=1 && checkNumber[1]!=4){
						var newColor = [colorChange[0]*1.3,colorChange[1]*1.3,colorChange[2]*1.3]
						//Verts
						draw_face([
							pos[0]+1.0, pos[1]+1.0, pos[2]-1.0,
							pos[0]-1.0, pos[1]+1.0, pos[2]-1.0,
					...
					//left
					if(checkNumber[2]!=1 && checkNumber[2]!=4){
						var newColor = [colorChange[0]*1.2,colorChange[1]*1.2,colorChange[2]*1.2]
						//Verts
						draw_face([
						pos[0]-1.0, pos[1]-1.0, pos[2]+1.0,
						pos[0]-1.0, pos[1]+1.0, pos[2]+1.0,  
					...
					//right
					if(checkNumber[2]!=1 && checkNumber[2]!=3 ){
						var newColor = [colorChange[0]*1.1,colorChange[1]*1.1,colorChange[2]*1.1]
						//Verts
						draw_face([
						pos[0]+1.0, pos[1]+1.0, pos[2]+1.0,
						pos[0]+1.0, pos[1]-1.0, pos[2]+1.0,
					...
					positionLayerArray.set(pushPosition,posIndex);
					colorLayerArray.set(pushColor,posIndex);
					posIndex+=pushColor.length;
		...
		chunk[chunkID].blockLayersCube[q].position = new Int16Array(posIndex);
		chunk[chunkID].blockLayersCube[q].color = new Uint8Array(posIndex);
		chunk[chunkID].blockLayersCube[q].position.set(positionLayerArray.slice(0,posIndex));
		chunk[chunkID].blockLayersCube[q].color.set(colorLayerArray.slice(0,posIndex));	

		
		//Set it as being drawn
		chunk[chunkID].blockLayersDrawn[q]=1;
		var endTime = new Date();
		var totTime=endTime - startTime;
		totalLayerTime += totTime;
		totalBlockHit+=blocksHit;
		blockHitCount+=1;
		if (longestLayerTime<totTime){
			longestLayerTime=totTime;
		}
		
		if(totTime>15){
			console.log("FATTY BOI:"+blocksHit+','+posIndex);
		}
		avgBlockHit = totalBlockHit / blockHitCount; 
		avgLayerTime = totalLayerTime / layerDraws;
}
...
draw_chunk =function(chunkID){
				...
				//delete
				/*for(var q=0;q<chunkZ;q++){
					
					//If the chunkLayer needs to be drawn draw it
					if(chunk[chunkID].blockLayersDrawn[q]==0){
						chunk_layer_draw(chunk[chunkID].coords[0],chunk[chunkID].coords[1],chunk[chunkID].coords[2],q);
					}
					
					//Push to our draw info 
						//If the position buffer has information
						if(chunk[chunkID].blockLayersCube[q].position.length>0){
							
								positionArray.set(chunk[chunkID].blockLayersCube[q].position,posInd);
								colorArray.set(chunk[chunkID].blockLayersCube[q].color,posInd);
								posInd+=chunk[chunkID].blockLayersCube[q].position.length;
				...
				posSend = new Int16Array(posInd);
				colSend = new Uint8Array(posInd);

				posSend.set(positionArray.slice(0,posInd));
				colSend.set(colorArray.slice(0,posInd));
				*/
...
self.addEventListener('message', function(e) {
	...
	switch(message.id){
		...
		break;
		//Reset the culling data (when loading a new map)
		case "reset":
		chunk=[];
		break;
		...
		view = Math.ceil(message.viewDist)+1;
		viewDist = message.viewDist;
		zLimit = message.zLimit;
		zView = message.zView;
		xx=-view;
		yy=-view;
		...
		culledList = new Uint16Array(len);
		chunkList.fill(0);
		culledList.fill(0);
		...
		//Generate list for layers being drawn
		for(var i=0;i<chunkZ;i++){
			chunkLayersDrawn[i]=1;
		}
		

		start=1;
		
		positionArray = new Int16Array(6*8*chunkList.length);
		colorArray = new Uint8Array(6*8*chunkList.length);
		positionLayerArray = new Int16Array(6*8*chunkXY*chunkXY);
		colorLayerArray = new Uint8Array(6*8*chunkXY*chunkXY);
		break;
...
var cullProc = setInterval(function(){
	...
	*/
		...
		while(going==1){
			...
			//If it needs to be culled
			if(chunkRef.chunkreDraw==1){
					draw_chunk(chunkID);

		//End checks to see if the chunk exists and needs to be culled		
			}
			}
	...
	/*
	  ___  _   _  __  __  _  _  _  _    ____  ____    __    _    _  ____  _  _  ___ 
	 / __)( )_( )(  )(  )( \( )( )/ )  (  _ \(  _ \  /__\  ( \/\/ )(_  _)( \( )/ __)
	( (__  ) _ (  )(__)(  )  (  )  (    )(_) ))   / /(__)\  )    (  _)(_  )  (( (_-.
	 \___)(_) (_)(______)(_)\_)(_)\_)  (____/(_)\_)(__)(__)(__/\__)(____)(_)\_)\___/
	
	*/
	
	//Loop through nearby chunks
	...
	for(var zCheck=-zView;zCheck<=zView;zCheck++){
		//Test if it needs to be drawn
		var chunkID = return_chunkID(camChunk[0]+xCheck,camChunk[1]+yCheck,camChunk[2]+zCheck);
		draw_chunk(chunkID);
...
if(cullingTime > 15 || drawingTime > 15){
	console.table({	
		"Culling time: " : cullingTime,
		"Drawing time: " : drawingTime,
		"Layers drawn: " : layerDraws,
		"Average layer draw time: "  :avgLayerTime,
		"Total layer draw time: " : totalLayerTime,	
		"Longest layer draw time: " : longestLayerTime,
		"Total blocks hit : " : totalBlockHit,
		"Average block hits : " : avgBlockHit,
	});
}
},1);
